<template>
  <v-container max-height="95vh" class="overflow-y-auto">
    <v-row class="pa-4" justify="space-between">
      ...
      <v-col cols="12" sm="auto" order="1" order-sm="2">
        <v-img src="@/assets/images/logo.png" style="width: 5cm"></v-img
      ></v-col>
    </v-row>
    <v-row justify="center" class="mt-5">
      <p class="text-h4 font-weight-bold" style="color: #dd0f2cff">
        {{ $t("inicio.bienvenido") }}
      </p>
    </v-row>
        ...
        <v-row justify="center" class="h-100">
          <v-col cols="8" lg="12" align-self="center">
            <v-row justify="center">
              <v-avatar
                class="ma-3"
                :color="BuscarModulo(index) ? 'red' : 'grey-lighten-2'"
    ...
    <v-row justify="center">
      <v-col style="text-align: center" v-if="informacion.PORCENTAJE_CURSO > 0">
        <v-btn
          ...
          style="color: white"
          >{{ $t("inicio.reintentar") }}</v-btn
        >
      </v-col>
      <v-col
        style="text-align: center"
        v-if="informacion.PORCENTAJE_CURSO == 0"
      >
        <v-btn
          rounded="xl"
          size="large"
