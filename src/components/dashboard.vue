<template>
  <v-container max-height="95vh" class="overflow-y-auto">
    <v-row class="pa-4" justify="space-between">
      ...
      <v-col cols="12" sm="auto" order="1" order-sm="2" class="py-0 pa-sm-3">
        <v-img src="@/assets/images/logo.png" style="width: 5cm"></v-img
      ></v-col>
    </v-row>

    <v-row>
      <v-col cols="6" sm="auto" class="pa-0 pa-sm-2">
        <p class="textoRojo font-weight-bold" @click="filtro()">
          {{ $t("dashboard.filtro") }}
          <v-icon color="red" icon="mdi-filter"></v-icon>
        </p>
      </v-col>
      ...
      >
        <template v-slot:item.MODULOS="{ item }">
          <div class="text-center">
            <v-avatar
              size="x-small"
              class="ma-1"
              :color="BuscarModulo(index, item) ? 'red' : 'grey-lighten-2'"
              v-for="index in niveles"
            >
              {{ index }}
            </v-avatar>
          </div>
        ...
        <template v-slot:item.ESTADO_PRUEBA="{ item }">
          <div class="text-center">
            <v-chip
              :color="
                item.ESTADO_PRUEBA == 'No registra'
                  ? 'grey'
                  : item.ESTADO_PRUEBA == 'Aprobada'
                    ? 'green'
                    : 'red'
              "
          ...
          <div class="text-center">
            <v-btn
              v-if="
                item.ENVIO == 0 &&
                item.ESTADO_PRUEBA == 'Aprobada' &&
                modulosid.every((num) => item.MODULOS.includes(num))
              "
              color="success"
              size="small"
              @click="certificado(item.IDUSUARIO)"
              >{{ $t("dashboard.enviar") }}</v-btn
            >
            <v-btn
              v-if="
                item.ENVIO == 1 &&
...
const niveles = computed(() =>
  parseInt(t("detalleCurso." + curso + ".modulos")),
);
...
const headers = [
  ...
  {
    key: "PORCENTAJE_PRUEBA",
    title: t("dashboard.campo.puntuacion"),
    align: "center",
  },
...
onMounted(async () => {
  const informacion1 = await axiosInstance.get("informacion", {
    headers: {
  ...
  switch (selectedFiltro1.value) {
    ...
    case 2:
      DataFilter.value = informacion.value.filter((item) => {
        return (
          item.ESTADO_PRUEBA == "Aprobada" &&
          modulosid.value.every((num) => item.MODULOS.includes(num))
        );
      });
      break;
    default:
